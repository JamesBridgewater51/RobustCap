...
class PureRNN(torch.nn.Module):
    ...
    def __init__(self, input_size: int, output_size: int, hidden_size: int, num_rnn_layer: int,
        ...
        """
        super().__init__()
        self.rnn = getattr(torch.nn, rnn_type.upper())(input_size, hidden_size, num_rnn_layer,
                                                       bidirectional=bidirectional, dropout=dropout, proj_size=output_size)
        if load_weight_file and os.path.exists(load_weight_file):
            self.load_state_dict(torch.load(load_weight_file, map_location=torch.device('cpu')))
            self.eval()
    ...
    def forward(self, x, init=None):
        ...
        """
        length = [_.shape[0] for _ in x]
        x = self.rnn(pack_padded_sequence(pad_sequence(x), length, enforce_sorted=False), init)[0]
